...
/**
 *  from https://csrc.nist.gov/files/pubs/fips/46-3/final/docs/fips46-3.pdf
*/
...
void generateRoundKeys(uint64_t key, uint64_t *roundKeys) {
    ...
    uint32_t left = (uint32_t) ((permutedKey >> 28) & 0x0FFFFFFF);
    uint32_t right = (uint32_t) (permutedKey & 0x0FFFFFFF);
    ...
    for (int i = 0; i < 16; i++) {
        ...
        for (int j = 0; j < 48; j++) {
            roundKey <<= 1;
            roundKey |= ((combinedKey >> (56 - PC2[j])) & 1);
        }
...
uint32_t permute(uint32_t substitutedHalfBlock) {
    ...
    for (int i = 0; i < 32; i++) {
        uint32_t bit = (substitutedHalfBlock >> (32 - P_TABLE[i])) & 1;
...
uint64_t expand(uint32_t halfBlock) {
    ...
    for (int i = 0; i < 48; i++) {
        result <<= 1;
        result |= (uint64_t) (halfBlock >> (32 - E[i])) & 1;
    }
    // printf("Expanded half block: %lx\n", result);
    return result;
}

uint32_t substitute(uint64_t mixed) {
    uint32_t output = 0;
    // printf("Expanded half block: %lx\n", mixed);
    for (int i = 0; i < 8; i++) {
        uint64_t sixBits = (mixed >> (42 - 6 * i)) & 0x3F;
        // printf("Six bits: %lx\n", sixBits);
...
void DESRound(uint64_t *data, uint64_t roundKey) {
    ...
    uint64_t mixed = expanded ^ roundKey;
    // printf("mixed: %lx\n", mixed);         
    uint32_t substituted = substitute(mixed);     
    uint32_t permuted = permute(substituted);    
    // printf("left: %x right: %x permutated: %x \n", left, right, permuted);
    uint32_t newRight = left ^ permuted;
    uint32_t newLeft = right;
    *data = (((uint64_t)newLeft) << 32) | (uint64_t)newRight;
    // printf("Round result: %lx  round key %lx\n", *data, roundKey);
}